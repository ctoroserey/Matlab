function [out] = foraging_test(Choice,OpportRate,Reward,Handling)
...
% suggestion (still need to plot). This fixed the probability dist.
...
if (sum(choice) == length(choice)) || (sum(choice) == 0) % if choices are one-sided
    ...
    for i = 1:length(b)
        ...
        [new.b,new.negLL] = fmincon(@negLL,b(i,:),[],[],[],[],[log(eps)],[-log(eps)],[],optimset('Algorithm','interior-point','Display','off'),choice,OR,Handle,Rwd);
...
out.SOC = OR.*Handle; % subjective opportunity cost (weighted)
...
function negLL = negLL(beta,choice,OR,Handle,Rwd)
p = probcalc(beta,OR,Handle,Rwd);
negLL = -sum((choice==1).*log(p) + (choice==0).*log(1-p));
end

function p = probcalc(beta,OR,Handle,Rwd)  
sOC = (OR.*Handle); % weighted opportunity cost
