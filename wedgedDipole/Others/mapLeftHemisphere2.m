...
function [leftLogmapPoints, leftInvLogmapPoints] = mapLeftHemisphere(p)
	...
	a_pixels = p.a_pixs; %p.a*pixelsPerDegree;
	...
	logmapOrigin = round(-leftLogmapExtent(1) + 1) - round(leftLogmapExtent(3) - 1)*1i;
	gridPointsW = gridPointsW + logmapOrigin;
	leftLogmapPoints = [gridPointsW; missingW + logmapOrigin];
	
	imageOrigin = round(p.nImageCols/2) + (round(p.nImageRows/2))*1i;
...
%     % Claudio's notes:
%     
%     - to get the lower hemifield: 
%             
%             HM = imag(leftInvLogmapPoints);
%             lowHem = leftInvLogmapPoints(HM < HM(1));
%
%     - what's tricky here is that I assumed a matrix, but this outputs a
%     complex vector. I'll have to modify one or the other (prob the latter).
